...
- Создаем функцию, которая будет менять значений нашей переменной и её триггер(кнопку):
...
При нажатии на кнопку значение переменной name сменится с mario на luigi и это перерендерится на сайт.

## Подведём итоги:

Мы создали reactive value. Для чего?

- Это необходимо, если мы собираемся изменять в дальнейшем данное значение, с обновлением интерфейса для юзера

- новое значение переменной может быть любым типом данных. Если изначально мы делали стринг, ничего не мешает обновить его в намбер или массив например